...
namespace audio
{
	struct Resonator
	{
		...
		void prepare(float sampleRate, int blockSize)
		{
			...
			const auto minFreqHz = 20.f;
			size = static_cast<int>(freqHzInSamples(minFreqHz, sampleRate));
			sizeF = static_cast<float>(size);

			wHead.prepare(blockSize, size);

			for(auto& r: ringBuffer)
				r.resize(blockSize, 0.f);
			for (auto& r : readHead)
				r.resize(blockSize, 0.f);
		}
			...
			float _feedback)
		{
			...
			const auto freq = noteInFreqHz2(42.f);
			const auto length = freqHzInSamples(freq, Fs);
			...
			for (auto ch = 1; ch < numChannels; ++ch)
				SIMD::copy(readHead[ch].data(), readHead[0].data(), numSamples);

			for (auto ch = 0; ch < numChannels; ++ch)
			{
				auto smpls = samples[ch];
				auto ring = ringBuffer[ch].data();
				const auto read = readHead[ch].data();
				...
				for (auto s = 0; s < numSamples; ++s)
				{
					...
					const auto sIn = smpls[s] + sOut * _feedback;
	...
	protected:
		WHead wHead;
		std::array<std::vector<float>, 2> readHead, ringBuffer;
